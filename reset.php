...
//if user want to enter reset page with null or empty email or token
if(!isset($_GET['email']) || !isset($_GET['token']) || empty($_GET['email']) || empty($_GET['token'])){
    redirect('index');
}

//fetch user info that have same token in url
if($stmt = mysqli_prepare($connection, 'SELECT username, user_email, token FROM users WHERE token=?')){
    ...
    //if user want to reset his password with invalid token and email pair.
    //return main page
